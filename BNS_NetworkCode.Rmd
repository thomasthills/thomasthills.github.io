---
title: "Behavioral Network Science Code Supplement"
...
plot(gbn, vertex.color = "cornflowerblue", vertex.size = 20, layout = layout_nicely)
...
maxweight <- max(E(cnetwork)$weight)
# What is the min weight
minweight <- min(E(cnetwork)$weight)
...
plot(cnetwork, edge.width=E(cnetwork)$weight, layout=layout_with_kk(cnetwork, weight=1/E(cnetwork)$weight))
# layout with fr uses weights by default to pull nodes together
plot(cnetwork, edge.width=E(cnetwork)$weight, layout=layout_with_fr)
```
...
```{r eigenvector}
eigen_centrality(cnetwork3)$vector

# plot node size proportional to eigenvector centrality
# make one plot window
par(mfrow=c(1,1))
# plot
plot(cnetwork3, vertex.size = eigen_centrality(cnetwork3)$vector*10, vertex.label.dist = 2)
...
### assign colors
V(cnetwork3)$color = pal(10)[igraph::degree(cnetwork3)] 
...
```{r errandomgraph}
ergp <- sample_gnp(40, .05)
ergm <- sample_gnm(40, 39) 
...
```{r smallworld}
swnp0 <- sample_smallworld(1,10,2, p=0)
swnp.1 <- sample_smallworld(1,10,2, p=.1)
swnp.2 <- sample_smallworld(1,10,2, p=.2)

# make three plots side by side
par(mfrow=c(1,3))
# plot each one
plot( swnp0, vertex.label = NA, vertex.color="chocolate1", main = "Small world p = 0", layout=layout_in_circle)
plot( swnp.1, vertex.label = NA, vertex.color="chocolate2", main = "Small world p = .1", layout=layout_in_circle)
plot( swnp.2, vertex.label = NA, vertex.color="chocolate3", main = "Small world p = .2", layout=layout_in_circle)
...
```{r configmodel}
cnetconfig <- rewire(cnetwork3, igraph::keeping_degseq(niter = 10))
